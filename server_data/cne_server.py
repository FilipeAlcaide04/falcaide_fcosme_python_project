...
def calcular_resultados_nacionais(dados):
    ...
    resultados = {
        'total': total_votos,
        'percentagens': {partido: (votos/votos_validos)*100 for partido, votos in total_votos.items()},
...
def get_resultados():
    ...
    for f in dados:
        if f['distrito'].lower() == distrito.lower() and f['freguesia'].lower() == freguesia.lower():
            votos_validos = f['votantes'] - f['brancos']
            percentagens = {partido: (votos/votos_validos)*100 for partido, votos in f['votos'].items()}
